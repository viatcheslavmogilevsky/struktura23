...
class ExampleStruktura < Struktura23::BaseSpec
  ...
  eks_cluster.has_many(:aws_eks_addon).where(cluster_name: eks_cluster.resolved.id).identify {|found_addon| found_addon.name}

  node_groups = eks_cluster.has_many(:aws_eks_node_group).where(cluster_name: eks_cluster.resolved.id).identify {|found_group| found_group.node_group_name}
  launch_template = node_groups.belongs_to(:aws_launch_template).where(name: node_groups.resolved.launch_template.name).identify {|found_lt| found_lt.id}
